...
from .models import MepSeries

logger = logging.getLogger(__name__)

_DOLAR_API_SPOT_URL = "https://dolarapi.com/v1/dolares/bolsa"
_ARGENTINA_DATOS_HISTORICAL_URL = "https://api.argentinadatos.com/v1/cotizaciones/dolares/bolsa"
_REQUEST_TIMEOUT = 10


def fetch_mep(cache: Cache) -> Optional[MepSeries]:
    """Fetch MEP spot + full historical series.
    ...
    """
    if cache.mep_is_fresh():
        series = cache.load_mep()
    ...
    cache.save_mep(historical, effective_spot, effective_date)
    logger.info("MEP updated: spot=%.2f as_of=%s, %d days of history", effective_spot, effective_date, len(historical))
    return MepSeries(data=historical, spot=effective_spot, as_of=effective_date)
...
def _fetch_spot() -> Tuple[Optional[float], Optional[date]]:
    try:
        ...
        logger.warning("Failed to fetch MEP spot from dolarapi: %s", exc)
        return None, None
...
def _fetch_historical() -> Optional[pd.Series]:
    try:
        ...
        logger.warning("Failed to fetch MEP history from argentinadatos: %s", exc)
        return None


def _fallback_to_stale_cache(cache: Cache) -> Optional[MepSeries]:
    series = cache.load_mep()
    cached_spot = cache.load_mep_spot()
    if series is not None and cached_spot is not None:
        spot, as_of = cached_spot
        logger.warning("MEP live fetch failed; using stale cache (as_of %s)", as_of)
        return MepSeries(data=series, spot=spot, as_of=as_of)
    logger.error("MEP fetch failed and no cache available")
    return None
